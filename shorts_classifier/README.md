...
**기기마다 두 라벨을 다 찍어라.** 한 기기가 숏폼만 찍으면 모델이 기기 모양으로 라벨을 맞힌다. `prepare.py` 가 이런 기기를 경고한다.
...
- **임계값은 `eval.py` 가 출력한 값을 반올림하지 말고 그대로 넣는다.** 추천값은 관측 점수 그 자체라,
  `0.45049` 를 `0.450` 으로 줄이면 사이에 있던 오탐이 들어와 정밀도 조건이 깨질 수 있다
