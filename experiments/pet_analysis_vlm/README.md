...
캐릭터 생성은 `../../pet_template/` 담당이고, 여기는 메타데이터만 뽑는다.
...
- 털색은 `../../pet_template/` 의 털색 추출(`extract_colors`)을 재활용하면 된다
...
> **이 디렉터리는 제품 경로가 아니다.** VLM 서버 방식은 GPU 비용과 개인정보 전송 때문에
> 채택하지 않았다. 실제 분석은 `../../pet_classifier/` 의 온디바이스 분류기가 담당한다.
> 성능 비교 기준선으로만 남겨 둔다.
